...
describe("The SnackScript analyzer", () => {
      ...
      makeClass(
        ...
        makeBlock([
          makeFunction(
            "greet",
            [],
            makeBlock([makePrintStatement(makeStringLiteral("Hello"))])
          ),
        ])
      ),
    ]);
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[0].class.name, "Person");
    assert.deepEqual(analyzed.statements[0].class.methods.length, 1);
  });
  ...
  it("correctly analyzes if statements", () => {
    ...
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[0].condition.op, "<");
    assert.deepEqual(analyzed.statements[0].body.length, 1);
  });
  ...
  it("correctly analyzes array collections", () => {
    ...
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[0].collection.collectionType, "Array");
    assert.deepEqual(analyzed.statements[0].collection.items.length, 3);
  });
  ...
  it("correctly analyzes tuple items in arrays", () => {
    ...
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[0].collection.items[0].kind, "Tuple");
    assert.deepEqual(
  ...
  it("correctly analyzes dictionary collections", () => {
    ...
    assert.deepEqual(analyzed.statements[0].collection.entries.length, 2);
  });
  ...
  it("correctly analyzes for loops", () => {
    const ast = makeProgram([
      makeForLoop(
        ...
        makeBlock([makePrintStatement(makeVarRef("i"))])
      ),
    ]);
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[0].kind, "ForLoop");
    assert.deepEqual(analyzed.statements[0].variable, "i");
  });
  ...
  it("correctly analyzes for-each loops", () => {
    const ast = makeProgram([
      ...
      makeForEach(
        "x",
        "numbers",
        makeBlock([makePrintStatement(makeVarRef("x"))])
      ),
    ]);
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[1].kind, "ForEach");
    assert.deepEqual(analyzed.statements[1].variables[0].name, "x");
  });
  ...
  it("correctly analyzes assignments", () => {
    ...
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[1].target.name, "x");
    assert.deepEqual(analyzed.statements[1].value.value, 42);
  });
  ...
  it("correctly analyzes add-assignments", () => {
    ...
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[2].target.name, "x");
    assert.deepEqual(analyzed.statements[2].value.value, 5);
  });
  ...
  it("correctly analyzes function calls", () => {
    const ast = makeProgram([
      makeFunction(
        "greet",
        ["name"],
        makeBlock([makePrintStatement(makeVarRef("name"))])
      ),
      makeCall("greet", [makeStringLiteral("World")]),
    ]);
    const analyzed = analyze(ast);
    assert.deepEqual(analyzed.statements[1].call.callee.name, "greet");
    assert.deepEqual(analyzed.statements[1].call.args.length, 1);
  });
  ...
  it("correctly analyzes binary expressions", () => {
    const ast = makeProgram([
      makeVarDecl([
        {
          id: "result",
          exp: makeBinaryExp(makeNumLiteral(10), "+", makeNumLiteral(20)),
        },
      ]),
    ]);
    const analyzed = analyze(ast);
  ...
  describe("detects semantic errors", () => {
    it("detects variable redeclaration", () => {
      ...
      assert.throws(() => analyze(ast), /already declared/);
    });
    ...
    it("detects use of undeclared variables", () => {
      ...
      assert.throws(() => analyze(ast), /not declared/);
    });
    ...
    it("detects return outside function", () => {
      ...
      assert.throws(() => analyze(ast), /can only appear in a function/);
    });
...
it("correctly analyzes expressions with floating-point values", () => {
  const ast = makeProgram([
    makeVarDecl([
      {
        id: "result",
        exp: makeBinaryExp(makeNumLiteral(3.5), "+", makeNumLiteral(2.7)),
      },
    ]),
  ]);
  const analyzed = analyze(ast);
...
it("correctly handles null or undefined expressions", () => {
  ...
  assert.strictEqual(analyzed.statements[0].declarations[0].initializer, null);
});
...
it("correctly analyzes expression statements with calls", () => {
  ...
  assert.equal(analyzed.statements[0].kind, "CallStatement");
  assert.equal(analyzed.statements[0].call.callee.name, "print");
});
...
it("correctly analyzes parenthesized expressions", () => {
    ...
    makeVarDecl([
      {
        id: "result",
        exp: makeParenExp(
          makeBinaryExp(makeNumLiteral(10), "+", makeNumLiteral(20))
        ),
      },
    ]),
  ]);
  ...
  assert.equal(analyzed.statements[0].declarations[0].initializer.op, "+");
  assert.equal(
    analyzed.statements[0].declarations[0].initializer.left.value,
    10
  );
...
it("correctly analyzes nested parenthesized expressions", () => {
  const ast = makeProgram([
    makeVarDecl([
      {
        id: "nested",
        exp: makeParenExp(makeParenExp(makeNumLiteral(42))),
      },
    ]),
  ]);
  ...
  assert.equal(analyzed.statements[0].declarations[0].initializer.value, 42);
});
    ...
    makeVarDecl([
      {
        id: "notComparison",
        exp: makeNegExp(
          makeBinaryExp(makeNumLiteral(5), "<", makeNumLiteral(10))
        ),
      },
    ]),
  ]);
...
it("correctly analyzes nested negation expressions", () => {
  const ast = makeProgram([
    makeVarDecl([
      {
        id: "doubleNegation",
        exp: makeNegExp(makeNegExp(makeBoolLiteral(false))),
      },
    ]),
  ]);
...
it("correctly analyzes call expressions within other expressions", () => {
  const ast = makeProgram([
    makeFunction(
      "getValue",
      [],
      makeBlock([makeReturnStatement(makeNumLiteral(42))])
    ),
...
it("correctly analyzes member access expressions", () => {
  ...
  assert.equal(analyzed.statements[2].value.kind, "MemberAccess");
  assert.ok(
    analyzed.statements[2].value.object,
    "MemberAccess should have an object property"
  );
  assert.equal(analyzed.statements[2].value.object.name, "obj");
  assert.ok(
    analyzed.statements[2].value.property,
    "MemberAccess should have a property field"
  );
  assert.equal(analyzed.statements[2].value.property, "property");
});
...
it("correctly analyzes assignments to object members", () => {
  ...
  assert.equal(analyzed.statements[1].target.kind, "MemberAccess");
  assert.equal(analyzed.statements[1].target.object.name, "obj");
  assert.equal(analyzed.statements[1].target.property, "property");
  assert.equal(analyzed.statements[1].value.value, 42);
});
...
it("correctly analyzes if statements with else-if parts", () => {
    ...
    makeIfStatement(
      ...
      [
        makeElseIf(
          ...
          makeBlock([
            makePrintStatement(makeStringLiteral("x is less than 20")),
          ])
        ),
        makeElseIf(
          makeBinaryExp(makeVarRef("x"), "<", makeNumLiteral(30)),
          makeBlock([
            makePrintStatement(makeStringLiteral("x is less than 30")),
          ])
        ),
      ],
      makeElse(
  ...
  assert.equal(analyzed.statements[1].kind, "IfStatement");
  ...
  assert.equal(analyzed.statements[1].elseIfs[0].condition.op, "<");
  assert.equal(analyzed.statements[1].elseIfs[0].condition.right.value, 20);
  assert.ok(
    Array.isArray(analyzed.statements[1].elseIfs[0].body),
    "elseIf body should be an array of statements"
  );

  assert.equal(analyzed.statements[1].elseIfs[1].condition.op, "<");
  assert.equal(analyzed.statements[1].elseIfs[1].condition.right.value, 30);
...
it("correctly analyzes if statements with a single else-if part", () => {
  ...
  assert.equal(analyzed.statements[1].elseIfs[0].condition.op, "==");
  assert.equal(analyzed.statements[1].elseIfs[0].condition.right.value, 2);
...
it("correctly handles null or undefined expressions in statements", () => {
  ...
  const ast = makeProgram([
    makeFunction("emptyReturn", [], makeBlock([returnWithoutExp])),
  ]);
...
it("correctly handles function calls with missing arguments", () => {
  ...
  const ast = makeProgram([callWithMissingArg]);
...
it("correctly filters only functions as class methods", () => {
  const classDef = {
    ...
    block: {
      ...
      statements: [
        ...
        makeSimpleFunction(
          "validMethod2",
          makeBlock([makePrintStatement(makeStringLiteral("I am also valid"))])
        ),
        ...
        makeFunction(
          "validMethod3",
          ["param"],
          makeBlock([makeReturnStatement(makeVarRef("param"))])
        ),
      ],
    },
  };
  ...
  const methodNames = analyzed.statements[0].class.methods.map(
    (m) => m.function.name
  );
  assert.deepEqual(
    methodNames.sort(),
...
it("exercises all expression type checks", () => {
  function testGetExpressionType(exp) {
    ...
    switch (exp.kind) {
      ...
      case "Class":
        return "CLASS";
      case "Collection":
        return exp.collectionType === "Array" ? "ARRAY" : "DICTIONARY";
      case "BinaryExpression":
        return exp.type || "UNKNOWN";
      case "FunctionCall":
        return exp.type || "ANY";
      case "Parameter":
        return exp.type || "ANY";
      case "UnaryExpression":
        if (
          exp.operator === "not" &&
          testGetExpressionType(exp.operand) === "BOOLEAN"
        ) {
          return "BOOLEAN";
        }
        return "UNKNOWN";
  ...
  assert.equal(
    testGetExpressionType({ kind: "IntegerLiteral", value: 42 }),
    "INTEGER"
  );
  assert.equal(
    testGetExpressionType({ kind: "FloatLiteral", value: 3.14 }),
    "FLOAT"
  );
  assert.equal(
    testGetExpressionType({ kind: "StringLiteral", value: "hello" }),
    "STRING"
  );
  assert.equal(
    testGetExpressionType({ kind: "BooleanLiteral", value: true }),
    "BOOLEAN"
  );
  assert.equal(
    testGetExpressionType({ kind: "Variable", name: "x", type: "INTEGER" }),
    "INTEGER"
  );
  assert.equal(testGetExpressionType({ kind: "Variable", name: "y" }), "ANY");
  assert.equal(
    testGetExpressionType({ kind: "Function", name: "myFunc" }),
    "FUNCTION"
  );
  assert.equal(
    testGetExpressionType({ kind: "Class", name: "MyClass" }),
    "CLASS"
  );
  assert.equal(
    testGetExpressionType({ kind: "Collection", collectionType: "Array" }),
    "ARRAY"
  );
  assert.equal(
    testGetExpressionType({ kind: "Collection", collectionType: "Dictionary" }),
    "DICTIONARY"
  );
  assert.equal(
    testGetExpressionType({ kind: "BinaryExpression", type: "INTEGER" }),
    "INTEGER"
  );
  assert.equal(testGetExpressionType({ kind: "BinaryExpression" }), "UNKNOWN");
  assert.equal(
    testGetExpressionType({ kind: "FunctionCall", type: "INTEGER" }),
    "INTEGER"
  );
  assert.equal(testGetExpressionType({ kind: "FunctionCall" }), "ANY");
  assert.equal(
    testGetExpressionType({ kind: "Parameter", type: "STRING" }),
    "STRING"
  );
...
it("handles unary expressions with unsupported operators or non-boolean operands", () => {
  ...
  assert.ok(
    numberAnalyzed,
    "Should analyze 'not' on a number without throwing"
  );
});
