...
export default function analyze(match) {
  ...
  const builder = match.matcher.grammar.createSemantics().addOperation("rep", {
    ...
    Statement_class(_jar, id, block) {
      ...
      const klass = core.cl(id.sourceString);
      context.add(id.sourceString, klass);
      context = context.newChildContext({ inClass: klass });
    ...
    Statement_function(_pot, id, _open, params, _close, block) {
      ...
      // // Safe mapping of parameter types
      // const paramTypes = fun.params.map((param) => param.type);
    ...
    Statement_foreach(_tempura, ids, _in, exp, block) {
      ...
      console.log(collection.type.kind);
    ...
    // Variable declaration: Type VarDecls
    // Statement_vardecl(varDecl) {
    //   return core.sequence(
    //     varDecls.children.map((decl) => {
    //       const varDecl = decl.rep();
    //       varDecl.type = core.anyType;
    //       mustBeAssignable(
    //         varDecl.initializer,
    //         { toType: core.anyType },
    //         { at: decl }
    ...
    ParamList(_iter) {
      return _iter.rep(); // Explicitly handle the iteration
    },

    NonEmptyParamList(params) {
      return params.rep(); // remember asIteration (maybe?)
    },
    ...
    ParamItem(_egg, id) {
      ...
      const param = core.variable(
        id.sourceString,
        isEgg ? core.intType : core.anyType
      );
    ...
    DictLit(_open, items, _close) {
      const entries = items.children.map((i) => i.rep());
      const keyType = entries.length > 0 ? entries[0].key.type : core.anyType;
      const valueType =
        entries.length > 0 ? entries[0].value.type : core.anyType;
    ...
    // Expression list for print: Exp ("," Exp)* | stringLit ("," (Exp | stringLit))*
    ExpList(expressions) {
    ...
    // Terminal handler - MUST BE INCLUDED
    _terminal() {
      return this.sourceString;
    },

    id(_char, _chars) {
      return this.sourceString; // Return the raw identifier string
    },
  });
